...
class Pages:
    ...
    async def show_page(self, page, *, first=False):
        ...
        p.append('')
        p.append('Press the 🔣 button to show the buttons for navigating.')
        self.embed.description = '\n'.join(p)
...
class EmbedPages:
    ...
    def __init__(self, ctx, author=None, *, embeds):
        self.bot = ctx.bot
        self.embeds = embeds
        self.message = ctx.message
        self.author = ctx.author
    ...
    async def show_page(self, page, *, first=False):
        ...
        self.current_page = page
        paged = 1
        embed = copy.copy(self.embeds[page - paged])
        p = []
        ...
        p.append('')
        p.append('Use the buttons to navigate through pages.')
        ...
        try:
            ...
            return await self.channel.send(embed=utils.fail("The search results content is too big for discord to handle, please search something else."))
        for (reaction, _) in self.reaction_emojis:
            if self.maximum_pages == 2 and reaction in ('\u23ed', '\u23ee'):
                continue
            await self.message.add_reaction(reaction)
